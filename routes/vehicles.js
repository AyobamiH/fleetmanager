...
r.post('/orgs/:orgId/vehicles', auth, requireOrg, async (req, res) => {
  const { name, plate, status, make, vehicleModel, year, vin, devideId, odometerKm, createdAt, updatedAt } = req.body || {};
  if (!name || !plate) return res.status(400).json({ error: 'name and plate required' });
  const doc = await Vehicle.create({ orgId: req.params.orgId, name, plate, status, make, vehicleModel, year, vin, devideId, odometerKm, createdAt, updatedAt });
